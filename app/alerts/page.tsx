...
export default async function NotificationsPage() {
  ...
  return (
    <CustomerShell active="/alerts" signedIn>
      <div className="rounded-3xl border border-black/10 bg-black/5 dark:border-white/10 dark:bg-white/5">
        <div className="border-b border-black/10 p-5 dark:border-white/10">
          <h1 className="text-2xl font-black">Alerts</h1>
          <p className="mt-1 text-sm text-black/50 dark:text-white/50">Likes, comments, broadcasts and booking updates open their exact destination when available.</p>
        </div>
        <div className="divide-y divide-black/10 dark:divide-white/10">
          {notifications.data.map((item) => {
            const href = mediaTargetFromNotification(item.metadata) || (item.type === "broadcast" ? "/inbox" : "/")
            return (
              <Link key={item.id} href={href} className={`block p-5 hover:bg-black/5 dark:hover:bg-white/5 ${item.isRead ? "" : "bg-emerald-900/20"}`}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-extrabold">{item.title || notificationTitle(item.type)}</p>
                    <p className="mt-1 text-sm leading-6 text-black/60 dark:text-white/60">{item.message}</p>
                    <p className="mt-2 text-xs text-black/40 dark:text-white/40">{new Date(item.createdAt).toLocaleString("en-KE", { timeZone: "Africa/Nairobi" })}</p>
                  </div>
          ...
          {!notifications.data.length ? <p className="p-8 text-center text-sm text-black/50 dark:text-white/50">No alerts yet.</p> : null}
        </div>
      </div>
    </CustomerShell>
  )
