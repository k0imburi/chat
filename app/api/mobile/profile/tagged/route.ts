...
export async function GET(request: Request) {
  ...
  const taggedMedia = await prisma.userMedia.findMany({
    where: {
      taggedUserIds: { array_contains: [targetUserId] },
  ...
  const data = taggedMedia
    .filter((media) => {
      const tags = serializeMediaTags(media);
      return (
        tags.taggedUserIds.includes(targetUserId) &&
        !media.copyrightStatus &&
        !media.reportStatus &&
        !media.isHiddenByOwner
      );
    })
    .map((media) => {
