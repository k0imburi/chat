...
import { z } from "zod"
import { markNotificationRead, deleteSingleNotification } from "@/lib/mobile-notifications"
...
export async function DELETE(request: Request, context: { params: Promise<{ notificationId: string }> }) {
  ...
  try {
    ...
    const result = await deleteSingleNotification(session.userId, params.notificationId)
    return NextResponse.json(result)
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ success: false, message: "Invalid request" }, { status: 400 })
    }
    logError("/api/mobile/notifications/[notificationId] DELETE", error)
    return NextResponse.json(
      { success: false, message: error instanceof Error ? error.message : "Failed to delete notification" },
      { status: 500 },
    )
  }
}
