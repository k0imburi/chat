...
export default async function InboxThreadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <CustomerShell active="/inbox" signedIn>
      <section className="overflow-hidden rounded-3xl border border-black/10 bg-black/5 dark:border-white/10 dark:bg-white/5">
        <div className="flex items-center gap-3 border-b border-black/10 p-4 dark:border-white/10">
          <Link href="/inbox" className="rounded-full border border-black/20 px-3 py-1.5 text-xs font-bold text-black/80 dark:border-white/20 dark:text-white/80">Back</Link>
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-black/50 dark:text-white/50">{broadcastOnly ? "Broadcast-only thread" : "Conversation"}</p>
          </div>
        </div>

        <div className="min-h-[460px] space-y-3 bg-black/[0.03] p-4 dark:bg-black/30">
          {chronological.map((message) => {
            const mine = message.senderId === viewer.userId
            return (
              <div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[82%] rounded-3xl px-4 py-3 text-sm ${mine ? "bg-[#25d366] text-white" : "bg-black/10 text-black dark:bg-white/10 dark:text-white"}`}>
                  {message.locked ? (
                  ...
                  <p className={`mt-2 text-[10px] ${mine ? "text-white/70" : "text-black/40 dark:text-white/40"}`}>{new Date(message.sentAt).toLocaleString("en-KE", { timeZone: "Africa/Nairobi" })}</p>
                </div>
              </div>
            )
          })}
          {!chronological.length ? <p className="py-16 text-center text-sm text-black/50 dark:text-white/50">No messages yet.</p> : null}
        </div>

        {broadcastOnly ? (
          <div className="border-t border-black/10 bg-amber-950/40 p-4 text-sm font-medium text-amber-400 dark:border-white/10">Replies are not available for broadcast messages.</div>
        ) : (
          <form action={sendTextMessage} className="grid gap-3 border-t border-black/10 p-4 dark:border-white/10 sm:grid-cols-[1fr_auto]">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-black/30 dark:text-white/30">Images are stored privately and only signed after entitlement checks.</p>
            </div>
  ...
  return (
    <div className="min-w-52">
      ...
      <form action={unlockMessage} className="mt-4">
        ...
        <Button size="sm" type="submit" variant="outline" className="border-black/20 bg-black/10 text-black hover:bg-black/20 dark:border-white/20 dark:bg-white/10 dark:text-white dark:hover:bg-white/20">Unlock with {unlockKind === "KEY" ? "Key" : "ChatCredit"}</Button>
      </form>
    </div>
  )
}
