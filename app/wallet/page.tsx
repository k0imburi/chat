      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {creditCards.map((card) => (
            <div key={card.key} className="rounded-3xl border border-black/10 bg-black/5 p-5 dark:border-white/10 dark:bg-white/5">
        ...
        <section className="rounded-3xl border border-black/10 bg-black/5 p-5 dark:border-white/10 dark:bg-white/5">
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-2">
            ...
            <div className="space-y-3 rounded-2xl bg-black/5 p-4 dark:bg-white/5">
              <div>
                <h3 className="font-black">Verify payout M-PESA number</h3>
                <p className="mt-1 text-xs leading-5 text-black/50 dark:text-white/50">Request an OTP, then verify it. Changing an existing payout number starts a 24-hour safety hold.</p>
              </div>
              <form action={requestPayoutOtpAction} className="flex gap-2">
                <input type="hidden" name="userId" value={user.userId} />
                <Input name="phoneNumber" placeholder="07XX XXX XXX" required />
                <Button type="submit" variant="outline" className="border-black/20 bg-transparent text-black hover:bg-black/10 dark:border-white/20 dark:text-white dark:hover:bg-white/10">Send OTP</Button>
              </form>
            ...
            {wallet.creditLedger.map((entry) => (
              <div key={entry.id} className="flex items-center justify-between gap-4 py-3 text-sm">
                <div>
                  ...
                  <p className="text-xs text-black/50 dark:text-white/50">{entry.createdAt.toLocaleString("en-KE", { timeZone: "Africa/Nairobi" })}</p>
                </div>
                <div className="text-right">
                  <p className="font-black tabular-nums">{entry.quantity ? `${entry.quantity > 0 ? "+" : ""}${entry.quantity}` : "—"}</p>
                  <p className="text-xs text-black/50 dark:text-white/50">{entry.currency} {Number(entry.value).toFixed(2)}</p>
                </div>
              </div>
            ))}
            {!wallet.creditLedger.length ? <p className="py-6 text-center text-sm text-black/50 dark:text-white/50">No credit activity yet.</p> : null}
          </div>
        </section>
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          <HistoryPanel title="Earning lots" empty="No creator earnings yet.">
            {wallet.earningLots.map((lot) => (
              <div key={lot.id} className="rounded-2xl bg-black/5 p-4 dark:bg-white/5 text-sm">
                <div className="flex items-center justify-between gap-3">
                  <p className="font-black">{lot.source.replaceAll("_", " ")}</p>
                  <span className="rounded-full bg-black/10 px-2.5 py-1 text-xs font-bold dark:bg-white/10">{lot.status}</span>
                </div>
                <p className="mt-2 font-bold tabular-nums">{lot.currency} {Number(lot.amount).toFixed(2)}</p>
                <p className="mt-1 text-xs text-black/50 dark:text-white/50">Available {lot.availableAt.toLocaleString("en-KE", { timeZone: "Africa/Nairobi" })}</p>
          ...
          <HistoryPanel title="Payouts" empty="No payouts have been created yet.">
            {wallet.payouts.map((payout) => (
              <div key={payout.id} className="rounded-2xl bg-black/5 p-4 dark:bg-white/5 text-sm">
                <div className="flex items-center justify-between gap-3">
                  <p className="font-black">{payout.currency} {Number(payout.amount).toFixed(2)}</p>
                  <span className="rounded-full bg-black/10 px-2.5 py-1 text-xs font-bold dark:bg-white/10">{payout.status}</span>
                </div>
                <p className="mt-1 text-xs text-black/50 dark:text-white/50">Created {payout.createdAt.toLocaleString("en-KE", { timeZone: "Africa/Nairobi" })}</p>
...
function MoneyCard({ label, value }: { label: string; value: number }) {
  return <div className="rounded-3xl border border-black/10 bg-black/5 p-5 dark:border-white/10 dark:bg-white/5"><p className="text-sm font-bold text-black/50 dark:text-white/50">{label}</p><p className="mt-3 text-3xl font-black tabular-nums">KES {value.toFixed(2)}</p></div>
}

function Info({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl bg-black/5 p-4 dark:bg-white/5"><p className="text-xs font-bold uppercase tracking-wide text-black/50 dark:text-white/50">{label}</p><p className="mt-1 font-extrabold">{value}</p></div>
}
  ...
  return (
    <section className="rounded-3xl border border-black/10 bg-black/5 p-5 dark:border-white/10 dark:bg-white/5">
      ...
      <div className="mt-4 space-y-3">
        {hasChildren ? children : <p className="py-6 text-center text-sm text-black/50 dark:text-white/50">{empty}</p>}
      </div>
    </section>
  )
}
